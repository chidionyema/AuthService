...
using auth.Models; // For User model
using auth.Db;    // For AuthContext and RefreshToken entity
using auth.Services; // For AuthService
 // Potentially for AuthContext if not from Db
...
namespace auth.Controllers
{
    ...
    public class AuthenticationController : ControllerBase
    {
        ...
        public AuthenticationController(
            ...
            AuthContext AuthContext, // Assuming this is the correct DbContext type name
        ...
        public async Task<IActionResult> RefreshToken([FromBody] RefreshTokenRequest request)
        {
            ...
            try
            {
                ...
                var storedToken = await _AuthContext.RefreshTokens
                ...
                if (storedToken.Expires < DateTime.UtcNow)
                {
                    ...
                    _AuthContext.RefreshTokens.Remove(storedToken); // Clean up expired token
                    await _AuthContext.SaveChangesAsync();
                    return Unauthorized(new { message = "Refresh token expired." });
                }

                // Refresh token is valid, remove old one, generate new ones
                _AuthContext.RefreshTokens.Remove(storedToken);
